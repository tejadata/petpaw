...
import { getHealthArticleContentFromFirestore } from "@/lib/data/health";
...
export function HealthArticleContent({ slug, fallbackContent }: Props) {
  const [content, setContent] = useState(fallbackContent || "");
  const [loading, setLoading] = useState(!fallbackContent);

  useEffect(() => {
    // If we already have content from static data, skip Firestore fetch
    if (fallbackContent) return;

    let cancelled = false;
    async function fetchContent() {
      setLoading(true);
      const article = await getHealthArticleContentFromFirestore(slug);
  ...
  if (!content) {
    return (
      <div className="rounded-2xl border bg-card p-6 sm:p-8 text-center text-muted-foreground">
        <p>Article content is loading. Please check back shortly.</p>
      </div>
    );
  }
